...
def main():
    ...
    if args.corrupt_label:
        assert len(args.T) != 0, "Need to pass noise matrix!"
        T = np.fromstring(args.T, sep=" ", dtype=float)
        if len(T) == 1:
            self_prob = T[0]
            T = np.ones((num_classes, num_classes)) * \
                ((1 - self_prob) / (num_classes-1))
            np.fill_diagonal(T, self_prob)
            # Note: this could potentially cause some numerical problem
        elif len(T) == num_classes ** 2:
            T = T.reshape(num_classes, -1)        
        else:
